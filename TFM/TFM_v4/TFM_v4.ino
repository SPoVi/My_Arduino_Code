 ...
 * 
 * Ultima fecha de modificacion: 25/08/2021
 */
...
{         
  ...
  if (!pox.begin()) {
      Serial.println("PulseOximeter initialize status: FAILED");
      for(;;);
  } else {
      Serial.println("PulseOximeter initizalize status: SUCCESS");
  }
  ...
  else
  {
    ...
    while(true){                                 // No sale del bucle. Hay que revisar el fallo de conexion y volver a cargar el programa
      
       
    }
    
  }
  Serial.print("Incio del tiempo desde el setup: ");
  Serial.println(g_reporting_period);
  g_tiempo_inicio = micros();   // Inicio del tiempo desde que temina el setup. Solo recoge 70 min. Cambiar a set Time. Incluir reloj.ino.
}
...
/*  
  ...
  mqttClient.loop();                      // Proces any outstanding MQTT messages. Tiene que ir después de la comprobacion del Wifi.

  */

  
  g_reporting_period = 1000;  // 1 segundo 10^6 microsegundos
  // Control de frecuencia de muestreo
  pox.update();
  if (millis() - g_tsLastReport > g_reporting_period){
    //g_tsLastReport = micros();  // Si se pone delante se tiene en cuenta lo que tarda en realizar las acciones
    Serial.print("Tiempo entre muestra y muestra: ");
    Serial.println(millis() - g_tsLastReport);
    Serial.print("Heart rate:");
    Serial.println(pox.getHeartRate());
    Serial.print("SpO2: ");
    Serial.println(pox.getSpO2());

    g_tsLastReport = millis();    // Si se pone detras se asegura que tras realizar las acciones habra el espacio seleccionado como mínimo.
    //updateReadings();
    //reportToMqtt();
...
void updateReadings(){
  ...
  pox.update();
  // MAX30100
  g_HR_value                  = pox.getHeartRate();
  g_SpO2_value                = pox.getSpO2();

  g_cantidad_datos_leidos++;

  // Mostrar datos ** SE PUEDE OMITIR
  
  Serial.print("Datos leidos (GSR, HR, SpO2): ");
  Serial.print(g_GSR_value);
  Serial.print(" ");
  Serial.print(g_HR_value);
  Serial.print(" ");
  Serial.println(g_SpO2_value);
}
  ...
  Serial.println("###### ENVIANDO MENSAJES MQTT ######");
    ...
    // in setup.
      ...
      if (true == g_GSR_readings_taken)
      {
         ...
         sprintf(g_mqtt_message_buffer,  "{\"ID:%s\":{\"GSR\":\"%f\",\"HR\":\"%f\",\"SpO2\":\"%f\",\"Número del paquete\":%i}}",  // Es necesario poner %s entre "\%s"\ por ser un string o char 
              g_cliente_ID, g_GSR_value, g_HR_value, g_SpO2_value, g_npaquete);
      }
